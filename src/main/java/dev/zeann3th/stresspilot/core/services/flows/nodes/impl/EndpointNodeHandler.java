...
/**
 * Handles the ENDPOINT node:
 * <ol>
 *   <li>Runs the pre-processor (sleep / inject / extract)</li>
 *   <li>Executes the endpoint via the executor factory</li>
 *   <li>Queues a {@link RequestLogEntity} for async persistence</li>
 *   <li>Runs the post-processor to extract/inject response fields</li>
 *   <li>Returns the next step ID based on success/failure</li>
 * </ol>
 */
...
public class EndpointNodeHandler implements FlowNodeHandler {
    ...
    private void process(String processorJson,
        ...
        try {
            ...
            if (CollectionUtils.isEmpty(proc)) return;
            ...
            if (proc.containsKey("extract") && prevResponse != null) {
                ...
                if (extractObj != null) {
                    ...
                    for (Map.Entry<String, Object> entry : extract.entrySet()) {
                        ...
                        if (value != null) variables.put(key, value);
                    }
                }
            }
